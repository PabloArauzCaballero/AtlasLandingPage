...
(function () {
  ...
  // En teléfonos no compensa: lo que cuesta dibujarlo se paga en la fluidez
  // de todo lo demás, y ahí el mapa es lo menos importante de la pantalla.
  if (innerWidth < 820) return;

  /* ── Three pesa ~600 KB. Antes se descargaba en toda visita, también en
     los teléfonos donde el mapa ni llega a dibujarse. Ahora se pide una
     sola vez y solo cuando la sección ya viene entrando. ── */
  let asked = false;
  function fetchThree() {
    if (asked) return;
    asked = true;
    const tag = document.createElement('script');
    tag.src = 'assets/js/vendor/three.min.js';
    tag.onload = function () { if (typeof THREE !== 'undefined') boot(); };
  ...
  if ('IntersectionObserver' in window) {
    ...
    addEventListener('load', fetchThree);
  }
